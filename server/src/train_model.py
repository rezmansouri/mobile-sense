...
class SensorDataset(Dataset):
    def __init__(self, data, labels, scaler=None, fit_scaler=False):
        ...
        if scaler is not None:
            ...
            # Reshape back
            self.data = (
                data_scaled.reshape(-1, 81, 9).transpose(0, 2, 1).reshape(-1, 729)
            )
...
def main():
    ...
    train_dataset = SensorDataset(
        data[train_idx], labels[train_idx], scaler, fit_scaler=True
    )
    val_dataset = SensorDataset(
        data[val_idx], labels[val_idx], scaler, fit_scaler=False
    )
    test_dataset = SensorDataset(
        data[test_idx], labels[test_idx], scaler, fit_scaler=False
    )
    ...
    config_best_models = {}  # Store best model info for each config
    all_results = {}
    ...
    for i, config in enumerate(hyperparam_grid):
        ...
        for epoch in range(num_epochs):
            ...
            # Save best model for THIS CONFIGURATION based on validation loss
            if val_loss < best_val_loss_config:
                best_val_loss_config = val_loss
                best_val_acc_config = val_acc
                best_epoch_config = epoch
                best_model_state_config = model.state_dict().copy()
                print(
                    f"  → New best for config {i+1}: val_loss={val_loss:.4f}, val_acc={val_acc:.2f}%"
                )

        # Save the best model for this configuration
        config_model_path = f"best_model_config_{i+1}.pth"
        torch.save(
            {
                "model_state_dict": best_model_state_config,
                "config": config,
                "val_loss": best_val_loss_config,
                "val_acc": best_val_acc_config,
                "epoch": best_epoch_config,
            },
            config_model_path,
        )
        ...
        config_results = {
            ...
            "best_epoch": best_epoch_config,
            "model_path": config_model_path,
        }

        all_results[f"config_{i+1}"] = config_results
        config_best_models[f"config_{i+1}"] = config_results

        print(f"Configuration {i+1} completed:")
        print(f"  Best val_loss: {best_val_loss_config:.4f}")
        print(f"  Best val_acc:  {best_val_acc_config:.2f}%")
        print(f"  Saved as: {config_model_path}")
        ...
        plt.title(f"Loss - Config {i+1} (Best: {best_val_loss_config:.4f})")
        plt.legend()
        ...
        plt.title(f"Accuracy - Config {i+1} (Best: {best_val_acc_config:.2f}%)")
        plt.legend()

        plt.tight_layout()
        plt.savefig(f"training_history_config_{i+1}.png", dpi=150, bbox_inches="tight")
        plt.close()

    # After all configurations are trained, select the OVERALL best based on validation accuracy
    print(f"\n{'='*50}")
    print("SELECTING OVERALL BEST CONFIGURATION")
    print(f"{'='*50}")
    ...
    for config_id, results in config_best_models.items():
        val_acc = results["best_val_acc"]
        print(f"{config_id}: val_acc = {val_acc:.2f}%")

        if val_acc > best_overall_val_acc:
            best_overall_val_acc = val_acc
            best_overall_config_id = config_id
            best_overall_model_path = results["model_path"]
            best_overall_config = results["config"]

    # Create a symbolic link or copy to indicate the overall best model
    overall_best_path = "best_model_overall.pth"
    import shutil

    shutil.copy2(best_overall_model_path, overall_best_path)

    print(f"\n{'='*50}")
    print("TRAINING COMPLETED")
    print(f"{'='*50}")
    print(f"Overall best configuration: {best_overall_config_id}")
    print(f"Best validation accuracy: {best_overall_val_acc:.2f}%")
    print(f"Best model: {best_overall_model_path}")
    print(f"Copied to: {overall_best_path}")
    print(f"Best hyperparameters: {best_overall_config}")

    # Save all results with overall best marked
    all_results["overall_best"] = {
        "config_id": best_overall_config_id,
        "val_accuracy": best_overall_val_acc,
        "model_path": overall_best_path,
    ...
    # Test the overall best model
    print(f"\nTesting overall best model ({best_overall_config_id}) on test set...")
    test_model(overall_best_path, test_dataset, device)
...
def test_model(model_path, test_dataset, device):
    ...
    test_results = {
        ...
        "config": config,
        "val_accuracy_at_best": checkpoint["val_acc"],
    }
